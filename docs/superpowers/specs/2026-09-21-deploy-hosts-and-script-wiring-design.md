...
`init` writes the file whenever it wires at least one script, overwrites a file whose marker names an older format, and leaves a current one alone. The apply step shows it as `Writing .kerstel/exec.cjs` and the diff view lists it as a new file with its full text, since it holds no secret.
...
A new `Launcher` row in the project group: `.kerstel/exec.cjs is current` passes; `.kerstel/exec.cjs is missing` is a problem with `Fix: kerstel init`; `.kerstel/exec.cjs is format 1, current is 2` warns with the same fix; `.kerstel/exec.cjs is not Kerstel's (no marker line)` warns and names the file. The row appears only when at least one script is wired in either form.
...
- **The product spec:** §6.2 points here for wiring, §6.3 drops the claim that the wizard wires build scripts so that build-time snapshotting works, since the launcher is what makes that true off the developer's machine, and §8 step 4 says the wizard writes the launcher.
...
- **`init`** (`init.test.ts`): writes the launcher, overwrites an older format, leaves a current one, warns about a `.gitignore` that hides it, and the self-check passes through the file just written.
- **`doctor`** (`doctor-checks.test.ts`, `init-status.test.ts`): each state in §6, the `Launcher` row's four outcomes, and no `Launcher` row in a project with nothing wired.
