...
- **Settings never come from the project.** The compiled binary is a Bun runtime and loads the working directory's `.env` before any Kerstel code runs, and §8's model is that this file is committed — so a cloned repository could otherwise set `KERSTEL_HOME`, `KERSTEL_KEYCHAIN_BACKEND`, `KERSTEL_IDLE_MS` or `KERSTEL_RELEASES_URL`. Every `KERSTEL_*` that an env file in the working directory NAMES is deleted from `process.env` before a command runs, and named once on stderr. The test is the name, not the value: a value match would have to reproduce Bun's loader exactly, and it does not — `KERSTEL_IDLE_MS=$ATTACK` expands and `"…\r"` decodes (both measured on 1.4.2), while the wizard's parser does neither, because it must round-trip a file it rewrites. Each difference is a smuggling route, so the check does not depend on the two agreeing. The cost is a setting the user made themselves being ignored while a project file names it; the warning says which file. The daemon is spawned with `~/.kerstel` as its cwd — Kerstel's own `0700` directory, not the user's home, which anyone's tooling may drop a `.env` into — and skips this check, since its environment is an allowlist built by an already-sanitised parent.
...
- A `get` whose stored value matches `^kerstel://` resolves through the daemon and returns the plaintext. Resolved values are memoized per process, keyed by name and checked against the reference. Under Bun the hook also resolves every reference in the environment at startup and writes the values back into the raw object, because `Bun.env` is that object and cannot be wrapped; see §8 of the [deploy hosts and script wiring spec](2026-09-21-deploy-hosts-and-script-wiring-design.md).
...
- **Child processes:** the hook injects the preload into `NODE_OPTIONS` (and Bun equivalents) in the env it exposes, so spawned node/bun children are covered and can resolve references of their own. Variables already present in the environment are handed to any child already resolved: building a child's envp reads `process.env` through the same trap application code uses, so the hook cannot tell the two apart, and a non-Node child (python, git, curl, ...) could not resolve a reference anyway. This matches level 1's stated boundary — a child is a process that runs code.
  - **Consequence, today, not a future concern:** envp construction enumerates *every* variable, so a single spawn resolves **every reference in the environment**, not only the ones the app actually reads — one audit row per secret, whether or not that secret was ever used. (Measured: the hook's per-process memo means that is once per secret per *process*, not per spawn, so the audit log undercounts spawns rather than the secrets exposed.) A `git` invocation in a dev server's file watcher resolves the whole vault slice the project references. The planned per-process approval gate has to account for this directly: an approval prompt per key per spawn is unusable, so the gate needs either resolution that is lazy *across* the envp boundary (a child env that still carries references, with the child's own hook resolving on read) or approvals scoped to a process tree rather than a single read.
...
- **The launcher and compound scripts:** as built, the prefix is `node .kerstel/exec.cjs -- `, a committed launcher that runs `kerstel exec` where the binary exists and the command unchanged where it does not, so a deploy host that runs `npm run build` needs no Kerstel. Each simple command of a compound script is wired on its own, after its leading `NAME=value` words. See the [deploy hosts and script wiring spec](2026-09-21-deploy-hosts-and-script-wiring-design.md).
...
- Build-time env snapshotting (e.g. `NEXT_PUBLIC_*` inlined into client bundles) works only when the build runs under the hook. The wizard wires build scripts too, through the launcher, so the same `build` runs hooked on the developer's machine and unchanged on a host that has no Kerstel and no references.
...
- **Access boundary:** a bearer token per daemon lifetime, minted by `daemon serve` on every start, kept at `~/.kerstel/session.token` (`0600`), deleted when the daemon stops, and compared in constant time on every request. The token value never enters any environment: `kerstel exec` hands the child `KERSTEL_TOKEN_FILE`, the file's path, and the hook's worker reads the file per request (retrying once on an unauthorized reply, so a daemon restart under a long-running app is transparent). Combined with the `0600` socket inside the `0700` home, that means only the owning user can read the token and only a caller holding it is served. The daemon does **not** verify peer UID: `node:net` exposes no peer credentials, and obtaining them would require a native module, which the single-self-contained-binary constraint rules out.
...
2. Parse `.env`, `.env.local` (and variants). Two kinds of `.env*` name are not env files and are skipped: templates (`.env.example`, `.sample`, `.template`, `.dist`, with or without `.local`), silently; and backup copies (`.bak`, `.orig`, `.old`, `.save`, `.backup`, `.swp`, `.swo`, `.tmp`, `.rej`, or a trailing `~`), each named in the output with a note that it may still hold plaintext and should be deleted, or renamed if it is a real env file. A backup copy would otherwise outrank `.env` in the precedence below and put its stale value in the vault. Within a file, every line that is neither blank, a comment, nor a parseable `KEY=value` is recorded as unsupported and counts as plaintext remaining: named by its key when the text before `=` looks like a variable name, by line number otherwise (prose, a base64 line, a name over 64 characters), so key material is never reprinted. An unquoted PEM block (`KEY=-----BEGIN ...-----` through `-----END ...-----`) is one unsupported value named by its key. Show findings. Per key: → project scope / → existing or new global key / leave plaintext (fine for non-secret URLs).
3. Encrypted backup of originals to `~/.kerstel/backups/<project>/<ts>/`, then rewrite files with references.
4. Wire the hook (§6.2): write the `.kerstel/exec.cjs` launcher and prefix each script's commands with it; show the `package.json` diff and the new file for approval.
