...
Open the vault without creating anything: no home, hook, token, or key is written, so `--dry-run` and every refusal leave the machine as they found it. With no vault on disk, there is nothing to restore; if the credential store still holds a key (`exists()`, read-only) that belongs to this home, a real run deletes that orphaned key. A key belongs to the home when the backend is the file backend (the key lives inside the home), when `KERSTEL_KEYCHAIN_SERVICE` rebinds the store item, or when the home is the default `~/.kerstel`. The native stores keep one machine-wide item, so under any other `KERSTEL_HOME` the key may be the one the real `~/.kerstel` needs, and it is never deleted. For each project in the `projects` table:
...
4. Delete the data key with the credential-store backend the vault was opened with, only when it belongs to this home (see §6.1); otherwise leave it and say so. It goes after the home so a failure here leaves a harmless orphaned key rather than a vault no key can open. The backends do not check the delete tool's exit code, so check `exists()` afterwards; if the key is still there, say so and exit 1 before removing the binary, so a re-run can delete the orphaned key.
