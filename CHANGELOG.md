...
- `kerstel init` now wires every command of a script, not only the first. Before, `node copy.mjs && next dev` became `kerstel exec -- node copy.mjs && next dev`, so `next dev` started without the hook and read the literal `kerstel://` reference, and a script starting with `NODE_ENV=x` broke because the assignment became the command. Now each command gets the wrapper after its leading `NAME=value` words, commands that never run JavaScript (`rm`, `echo`, `mkdir`, and the like) are left alone, and a script `init` cannot wire safely (one with `cd`, a redirection, a subshell, or an unbalanced quote) is skipped and named with the reason. Re-running `init` finishes a script that was wired only in part. `kerstel doctor` names partly wired and skipped scripts instead of counting them as wired, and `kerstel uninstall` unwraps every command.
...
- Kerstel no longer takes its own settings from the project it is protecting. Kerstel is a Bun binary, so it loads the `.env` of the directory you run it in, and Kerstel's model is that this file is committed: a repository you cloned could set `KERSTEL_HOME` to a directory inside itself, force the weaker key-file credential store, or stop the daemon ever relocking. Any `KERSTEL_*` that an env file there names is now ignored and named on stderr, whatever its value, so a setting you made yourself is ignored too while that line is in the file. The resolver daemon runs from `~/.kerstel`, so no env file reaches it. Set these in your shell or shell profile instead.
- The daemon's session token no longer travels in any environment. `kerstel exec` now gives a wired script `KERSTEL_TOKEN_FILE`, the path of the `0600` token file, instead of `KERSTEL_TOKEN`, and the runtime hook reads the file when it needs it. Before, the token sat in every hooked process's environment and every child's, where `console.log(process.env)`, a crash reporter, or `ps -E` could show it, and it was never rotated. The daemon now mints a fresh token every time it starts and deletes it when it stops, so no valid token exists while nothing is listening. A running app keeps working across a daemon restart. A `KERSTEL_TOKEN` inherited from a process an older Kerstel had hooked is dropped rather than passed on.
...
- The [changelog](https://kerstel.dev/changelog) page keeps the unreleased section collapsed until you expand it, and marks the newest release as the latest.
