...
`kerstel init` wires this up for you. Every script in `package.json` becomes `kerstel exec -- <your original command>` — npm lifecycle scripts such as `postinstall` and `prepare` are never wrapped, because that would make `npm install` itself depend on Kerstel — and a Bun project also gets the hook added to `preload` in `bunfig.toml`. Kerstel writes the hook's files to `~/.kerstel/hook` the first time any `kerstel` command runs, and the file everything points at is `~/.kerstel/hook/preload.cjs`.
...
The path written into `bunfig.toml` is absolute: `bunfig.toml` does not expand `~`. It is also per-machine, so the committed path is not the one that works on a teammate's laptop — every developer on a Bun project runs `kerstel init` once after cloning, and it replaces the stale path with theirs.

### Wiring it by hand

A project with no `package.json` scripts to wrap can point at the hook itself. For Bun, add it to `preload` in `bunfig.toml`:

```toml
preload = ["/Users/you/.kerstel/hook/preload.cjs"]
```

Write the full path there (on Linux it is under `/home/you`), since `bunfig.toml` does not expand `~`.

For Node, set `NODE_OPTIONS` on the scripts that need it:

```bash
NODE_OPTIONS="--require /Users/you/.kerstel/hook/preload.cjs" npm run dev
```
