...
The daemon is started with a small environment of its own: `PATH`, `HOME`, locale, the session bus and display on Linux, and Kerstel's own `KERSTEL_*` settings. It never inherits the environment of the script that needed it. That keeps your project's variables, an outer script's already-resolved values, and runtime flags such as `BUN_OPTIONS` or `NODE_OPTIONS` out of the process that holds the vault key.
