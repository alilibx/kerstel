...
It shows every variable it found up front, grouped by where it suggests putting it — the vault for this project, the vault shared across all your projects, or left as plain text — with key names, value sizes and shapes, but never the values themselves. Then it asks:
...
3. **"Apply these changes?"**, under the full plan and a line-for-line diff of every file it means to change — the backup, the vault entries, the `.env` rewrites, and the wiring, all masked. **Apply** is the default; **Show the full diff first** prints it and asks again; **Cancel** writes nothing.
